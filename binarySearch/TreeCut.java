...
public class TreeCut {
	...
	private static int treeCut(int start, int end, long norma) {
		...
		while(start <= end) {
			...
			if(gain >= norma) {
				result = (int)mid;	//자르는 값은 int값을 넘지않는다
				start = (int)mid +1;
			}else {
				end = (int)mid-1;
			}
...
/*
주의사항 : 나무 길이가 2,000,000,000 이므로, int값으로 담을수 있다.
left에 담길 값이 최대 	2,000,000,000,000,000 => 
long 최대값인 			9,223,372,036,854,775,807 를 넘지않는다. -> 비교값은 long이어야 한다.
 */
