...
public class Backj17496_StarFruit {

	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st = new StringTokenizer(br.readLine());
		int summer = Integer.parseInt(st.nextToken());
		int fruitGrow = Integer.parseInt(st.nextToken());
		int lands = Integer.parseInt(st.nextToken());
		int price = Integer.parseInt(st.nextToken());
		
		//1일부터 시작한다는점에 유의하자.
		int harvest = Math.floorDiv((summer-1),fruitGrow);
		System.out.println(harvest*lands*price);
